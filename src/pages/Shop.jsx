...
//       `http://localhost:8080/products?_page=${filterObj.current_page}&_limit=20`
...
//     return { productsData: [], productsLength: 0, page: 1 };
//   }
//   // /posts?views_gte=10
...
//         <div className="grid grid-cols-4 px-2 gap-y-4 max-lg:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 shop-products-grid gap-3">
...
export const shopLoader = async ({ request }) => {
  ...
  try {
    const response = await axios.get("http://localhost:8080/products/search", {
      params: {
        productName: filterObj.search,

      },
    });
    console.log(response.data);

    return { productsData: response.data, productsLength: response.data.length };
  } catch (error) {
    console.log(error.response);
    return { productsData: [], productsLength: 0 };
  }
};
