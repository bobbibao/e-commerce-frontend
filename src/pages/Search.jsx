...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await axios(
        `http://localhost:8080/products?q=${e.target.search.value}&_page=${currentPage}&_limit=12`
      );
  ...
  const handleSearchPagination = async () => {
    try {
      const response = await axios(
        `http://localhost:8080/products?q=${searchTerm}&_page=${currentPage}&_limit=12`
      );
      const data = response.data;
      setProducts(data);
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-4 px-2 max-w-7xl mx-auto gap-y-4 max-lg:grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 shop-products-grid gap-3">
